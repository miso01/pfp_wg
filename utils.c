...
FILE *open_aux_file_num(
    const char *base, const char *ext, const int num, const char *mode
) {
    char *name;
...
mFile *mopen_aux_file(const char *base, const char *ext, int nsegs) {
    ...
    if (f->f == NULL)
        die("Error opening multiple file");
    return f;
}
...
size_t mfread(void *vptr, size_t size, size_t nmemb, mFile *f) {
    ...
    while (1) {
        ...
        if (s == 0) {
            if (ferror(f->f))
                die("Error reading file segment");
            if (++f->cur >= f->nsegs)
                return read; // no more files: return the items actully read
            if (fclose(f->f) != 0)
                die("Error closing file segment");
            f->f = open_aux_file_num(f->base, f->ext, f->cur, "rb");
            if (f->f == NULL)
                die("Error opening new file segment");
        } else {
            read += s;
            nmemb -= s;
            if (nmemb == 0)
