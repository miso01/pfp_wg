...
#define chr(i)                                                                 \
    (cs == sizeof(int_t) ? ((int_t *)s)[i]                                     \
                         : (cs == sizeof(int_text) ? ((int_text *)s)[i]        \
                                                   : ((unsigned char *)s)[i]))
...
#define DEPTH                                                                  \
    0 // compute time and size of reduced problem for each recursion call
...
#define EMPTY_STRING                                                           \
    0 // check if there is an empty string in the input collection
...
void compute_lcp_phi_sparse(
    int_t *s, uint_t *SA1, uint_t *RA, int_t *LCP, int_t *PLCP, uint_t n1,
    ...
    for (i = 0; i < n1 - 1; i++) {
        ...
        if (LCP[i] == (int_t)n1 - 1)
            ...
            l -=
                max(RA[i + 1] - RA[i], RA[LCP[i] + 1] - RA[LCP[i]]
                ); // LCP[i] stores the distance of i-th suffix to its successor
    }
...
void getBuckets_k(
    int_t *s, uint_t *bkt, uint_t n, unsigned int K, int end, int cs
) {
    uint_t i, sum = 0;
...
void putSuffix0(
    uint_t *SA, int_t *s, uint_t *bkt, uint_t n, unsigned int K, int_t n1,
    int cs
) {
    uint_t i, j;
...
void putSuffix0_generalized(
    uint_t *SA, uint_t *s, uint_t *bkt, uint_t n, unsigned int K, int_t n1,
    int cs, uint_t separator
) {
    uint_t i, j;
    ...
    SA[tmp] =
        SA[0] - 1; // insert the last separator at the end of bkt[separator]
}

void putSuffix0_generalized_LCP(
    uint_t *SA, int_t *LCP, uint_t *s, uint_t *bkt, uint_t n, unsigned int K,
    int_t n1, int cs, uint_t separator
) {
    uint_t i, j;
    int_t l;
...
void putSuffix0_generalized_DA(
    uint_t *SA, int_t *DA, uint_t *s, uint_t *bkt, uint_t n, unsigned int K,
    int_t n1, int cs, uint_t separator
) {
    uint_t i, j;
...
void putSuffix0_generalized_LCP_DA(
    uint_t *SA, int_t *LCP, int_t *DA, uint_t *s, uint_t *bkt, uint_t n,
    unsigned int K, int_t n1, int cs, uint_t separator
) {
    uint_t i, j;
    int_t l;
...
void induceSAl0(
    uint_t *SA, int_t *s, uint_t *bkt, uint_t n, unsigned int K, int_t suffix,
    int cs
) {
    uint_t i, j;
...
void induceSAl0_generalized(
    uint_t *SA, uint_t *s, uint_t *bkt, uint_t n, unsigned int K, int_t suffix,
    int cs, uint_t separator
) {
    uint_t i, j;
...
void induceSAs0_generalized(
    uint_t *SA, uint_t *s, uint_t *bkt, uint_t n, uint_t K, int_t suffix,
    int cs, uint_t separator
) {
    uint_t i, j;
...
void induceSAl0_generalized_LCP(
    uint_t *SA, int_t *LCP, uint_t *s, uint_t *bkt, uint_t n, unsigned int K,
    int cs, uint_t separator
) {
    uint_t i, j;
        ...
        if (SA[i] != U_MAX) {
            ...
            if (LCP[i] == I_MIN) { // is a L/S-seam position
                ...
                if (SA[bkt[chr(SA[i])] - 1] < n - 1)
                    while (chr(SA[i] + l) == chr(SA[bkt[chr(SA[i])] - 1] + l))
                        ++l;
                LCP[i] = l;
            }
#if RMQ_L == 1
            ...
            if (top > STACK_SIZE_L) { // if stack is full
                ...
                for (j = 0; j < (int_t)K; j++) {
                    ...
                    if (STACK[end - 1].idx < tmp[j] + 1) {
                        ...
                        if (curr < top) {
                            stack_push_k(
                                STACK, &end, STACK[curr].idx, STACK[curr].lcp
                            );
                            curr++;
                        }
                    }
...
void induceSAs0_generalized_LCP(
    uint_t *SA, int_t *LCP, uint_t *s, uint_t *bkt, uint_t n, uint_t K, int cs,
    uint_t separator
) {
    uint_t i, j;
        ...
        if (top >= STACK_SIZE_S) {
            ...
            for (j = K - 1; j >= 0; j--) {
                ...
                if (tmp[j] < STACK[end - 1].idx) {
                    ...
                    while (STACK[curr].idx > tmp[j] && curr < top)
                        curr++;
                    if (curr >= top)
                        break;
...
void induceSAl0_generalized_DA(
    uint_t *SA, int_t *DA, uint_t *s, uint_t *bkt, uint_t n, unsigned int K,
    int cs, uint_t separator
) {
    uint_t i, j;
...
void induceSAs0_generalized_DA(
    uint_t *SA, int_t *DA, uint_t *s, uint_t *bkt, uint_t n, uint_t K, int cs,
    uint_t separator
) {
    uint_t i, j;
...
void induceSAl0_generalized_LCP_DA(
    uint_t *SA, int_t *LCP, int_t *DA, uint_t *s, uint_t *bkt, uint_t n,
    unsigned int K, int cs, uint_t separator
) {
    uint_t i, j;
...
void induceSAs0_generalized_LCP_DA(
    uint_t *SA, int_t *LCP, int_t *DA, uint_t *s, uint_t *bkt, uint_t n,
    uint_t K, int cs, uint_t separator
) {
    uint_t i, j;
...
void putSubstr0(
    uint_t *SA, int_t *s, uint_t *bkt, uint_t n, unsigned int K, int cs
) {
...
/*****************************************************************************/
void putSubstr0_generalized(
    uint_t *SA, uint_t *s, uint_t *bkt, uint_t n, unsigned int K, int cs,
    uint_t separator
) {
    uint_t i, cur_t, succ_t;
    ...
    for (i = n - 2; i > 0; i--) {
        ...
        if (cur_t == 0 && succ_t == 1) {
            ...
            if (chr(i) == separator)
                SA[++bkt[chr(p)]] =
                    0; // removes LMS-positions that induces separator suffixes
...
void induceSAl1(int_t *SA, int_t *s, int_t n, int_t suffix, int cs) {
    int_t h, i, j, step = 1;
    ...
    for (i = 0; i < n; i += step) {
        ...
        int_t c2;
        int_t isL1 =
            (j + 1 < n - 1) && (c1 > (c2 = chr(j + 2)) || (c1 == c2 && c1 < i)
                               ); // is s[SA[i]] L-type?
        if ((!suffix || !isL1) && i > 0) {
...
void induceSAs1(int_t *SA, int_t *s, int_t n, int_t suffix, int cs) {
    int_t h, i, j, step = 1;
...
int_t SACA_K(
    ...
    if (name_ctr < n1)
        depth +=
            SACA_K((int_t *)s1, SA1, n1, 0, m - n1, sizeof(int_t), level + 1);
...
int_t gSACA_K_LCP(
    uint_t *s, uint_t *SA, int_t *LCP, uint_t n, unsigned int K, int cs,
    uint_t separator, int level
) {
    uint_t i;
    uint_t *bkt = NULL;
    uint_t m = n;
...
// stage 3: induce SA(S) from SA(S1).
    ...
    /*
      if(!lcp_array_check((unsigned char*)s, SA1, LCP, n, cs,
      separator))printf("isNotLCP (lms)!!\n"); else printf("isLCP (lms)!!\n");
    */
...
int_t gSACA_K_DA(
    uint_t *s, uint_t *SA, int_t *DA, uint_t n, unsigned int K, int cs,
    uint_t separator, int level
) {
    uint_t i;
    uint_t *bkt = NULL;
...
int_t gSACA_K_LCP_DA(
    uint_t *s, uint_t *SA, int_t *LCP, int_t *DA, uint_t n, unsigned int K,
...
int gsacak(unsigned char *s, uint_t *SA, int_t *LCP, int_t *DA, uint_t n) {
    ...
    if ((LCP == NULL) && (DA == NULL))
        ...
        return gSACA_K_LCP_DA(
            (uint_t *)s, SA, LCP, DA, n, 256, sizeof(char), 1, 0
        );
}
...
int gsacak_int(
    ...
    if ((LCP == NULL) && (DA == NULL))
        ...
        return gSACA_K_LCP_DA(
            (uint_t *)s, SA, LCP, DA, n, k, sizeof(int_text), 1, 0
        );
}
